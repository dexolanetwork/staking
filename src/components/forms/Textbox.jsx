...
const LoginForm = ({ amountInEther, setAmountInEther, amountInRBO, setAmountInRBO }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      const tx = await sendEtherToContract(
  ...
  const handleOnchangeInEther = (e) => {
    ...
    if (e.target.value >= 0.01) setError('');
  ...
  const handleOnchangeInRBO = (e) => {
    ...
    if (e.target.value >= 100) setError('');
  };

  return (
    <div className="flex bg-white/10 items-center justify-center w-full">
      <div className="w-full p-8 space-y-6 rounded-lg shadow-lg">
        <h2 className="text-2xl font-semibold text-gray-300">In Progress</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="amountInEther" className="block text-sm font-medium text-yellow-500">ETH</label>
            <input
              type="number"
              id="amountInEther"
              value={amountInEther}
          ...
          <div>
            <label htmlFor="amountInRBO" className="block text-sm font-medium text-yellow-500">RBO</label>
            <input
              type="number"
              id="amountInRBO"
              value={amountInRBO}
