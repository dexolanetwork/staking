...
const HeroPage = () => {
  ...
  const handleClick      = () => navigate('/presale');
  ...
  return (
    <section className="flex items-center justify-center bg-gradient-to-r from-indigo-600 to-purple-600 py-24 min-h-screen relative overflow-hidden">
      <div className="text-center text-white px-4 md:px-12 md:pl-18">
        <div className="max-w-5xl mx-auto text-center space-y-6 relative z-10">
          ...
          <div className="space-x-4">
            <button
              onClick={handleClick}
              className="bg-yellow-400 text-black py-3 px-8 rounded-xl text-lg font-medium shadow-md hover:bg-yellow-500 transition-colors mt-8"
            >
              Join Presale
            </button>
            <button
              onClick={walletAddress ? handleDisconnect : handleConnect}
              className="hidden md:inline border-[2px] border-yellow-400 text-white font-bold py-2 px-8 rounded-xl text-lg shadow-md hover:bg-yellow-500 transition-colors mt-8"
            >
      ...
      <div className="hidden md:block mt-8">
        <img src="/images/4.png" alt="Hero" className="max-w-full h-auto mx-auto" />
      </div>
    </section>
  );
};
